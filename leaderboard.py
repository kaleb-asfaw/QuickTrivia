...
def update_global_best(score, username):
    ...
    if global_best_doc.exists:
        ...
        global_best_scores = sorted(global_best_scores, key=lambda x: x['score'], reverse=True)[:3]
    else:
...
def get_leaderboard():
    ...
    return leaderboard_ref.order_by('scores', direction=firestore.Query.DESCENDING).stream()
